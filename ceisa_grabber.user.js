...
// @description To grab all available data
// @include     *
// @version     1
// @grant       GM_addStyle
...
// @resource    homepagev4  http://192.168.146.250/autograbber/grabber.php
// ==/UserScript==
...
function table_insert_data(tablename, data){
  ...
  for(var i=0; i<data.length; i++){
    var row='<tr><td>'+(lastid+1)+'.</td><td class="editable">'+data[i].no_pib+'</td><td class="editable">'+data[i].tglpib+'</td><td>'+data[i].importir+'</td><td>'+data[i].jalur+'</td><td><label><input type="checkbox" value="'+(lastid)+'">&nbsp;</label></td></tr>';
    rows+=row;
    lastid++;
  }
  ...
  if ( (curr_page <= total_page) && (curr_page > pageLoaded)) {
    ...
    if (data.length > 0) {
      ...
      for (var i=0; i<data.length/13; i++) {
        ...
        csvData.push([
          csvData.length, no_pib, tgl_pib, imp, jalur
          ]);
