// ==UserScript==
// @name        apps_mager_grabber
// @namespace   my_testcase
// @description To grab all available data from apps manager
// @include     *
// @version     1
// @grant       GM_addStyle
...
// @resource    homepagev4  http://192.168.146.250/autograbber/apps_mager_grabber.php
// ==/UserScript==
...
function table_insert_data(tablename, data){
  ...
  for(var i=0; i<data.length; i++){
    var row='<tr><td>'+(data[i].no_user)+'.</td><td class="editable">'
          +data[i].nama+'</td><td class="editable">'
          +data[i].nip+'</td><td>'
          +data[i].username+'</td><td>'
          +data[i].pangkat+'</td><td>'
          +data[i].jabatan+'</td><td>'
          +'<label><input type="checkbox" value="'+(lastid)+'">&nbsp;</label></td></tr>';
    rows+=row;
    lastid++;
  }
...
function grab_data_pib() {
  // grab data tentang halaman ke brp dari berapa
  var curr_page = $("input.z-paging-inp")[0].value;
  console.log(curr_page);

  // total halaman
  var total_page = /^\/\s+(\d*)$/.exec($("span.z-paging-text")[1].innerText)[1];
  ...
  var btn_next = $("button.z-paging-next");
  ...
  if ( (curr_page <= total_page) && (curr_page > pageLoaded)) {
    ...
    if (data.length > 0) {
      console.log("Di halaman ini ada " + data.length + " row data");

      if (data.length == 145) {
        console.log("Scrolling to get more...");
        // more data is available
        var v = $('.z-listbox-body')[0];
        v.scrollTo(0, 900);

        setTimeout(grab_data_pib, 4000);
        // refresh until number changes
      ...
      while (i<data.length) {
        ...
        var no_user = data[i++].innerText;
        var nama = data[i++].innerText;
        var nip = data[i++].innerText;
        var username = data[i++].innerText;
        var pangkat = data[i++].innerText;
        var jabatan = data[i++].innerText;
        ...
        csvData.push([
          no_user, nama, nip, username, pangkat, jabatan
          ]);
...
$(document).ready(function(){
      ...
      case 'btn_mulai':
        {
          ...
          csvData = [];
          csvData.push([
            "no", "no_pib", "tgl_pib", "importir", "jalur"
          ]);
          grab_data_pib();
          /*var curr_page = $("input.z-paging-inp")[0].value;
          console.log(curr_page);
          ...
          if (data.length > 0) {
            console.log("Di halaman ini ada " + data.length/13 + " row data");

            for (var i=0; i<data.length/13; i++) {
              var no_pib = data[i*13+0].innerText;
              var tgl_pib = data[i*13+1].innerText;
              var imp = data[i*13+2].innerText;
              var jalur = data[i*13+7].innerText;
  ...
  //==========================ENTRY POINT===============================================================================================
  if(!home.exists()){
    var contents=GM_getResourceText('homepagev4');
    //alert(contents);
